...
router.get("/check-email", async (req, res) => {
    try {
        ...
        const user = await User.findOne({ email }).select("emailVerified verificationMethod");
        if (!user) {
            // ไม่เจอ user ก็ส่งค่า false กลับ (จะได้โชว์ badge แดงเฉย ๆ)
            return res.json({ ok: true, emailVerified: false, verificationMethod: null });
        }
